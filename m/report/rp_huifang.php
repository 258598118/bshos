...
<?php
	...
	) ))
	{
		...
		{
			$data [$k] ["总"] = $db->query ( "select count(*) as c  from $table where huifang REGEXP '<b>".@date('Y-m-d',$v [0]).".{5,10}'", 1, "c" );
...
<?php foreach ($final_dt_arr as $k => $v) { ?>
	<tr>
			...
			<td align="center"><?php echo $data[$k]["总"]; ?></td>
            <?php   foreach ($kf_arr as $me => $num) { ?>
