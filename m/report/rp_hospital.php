...
#p_border {float:left; width:202px; height:12px; border:1px solid #408080; background:white; }
...
<header class="jumbotron subhead"  style="margin-bottom: 20px;">
    <ul class="breadcrumb">
          ...
          <li class="active">数据横向对比</li>
    </ul>
</header>

<div style="text-align:center; padding:20px 0 5px 0;">
	类型：<input type="button" class="btn" value="整个医院" onclick="location='?'">&nbsp;
	<input type="button" class="btn" value="网络" onclick="location='?type=2'">&nbsp;
	<input type="button" class="btn" value="电话" onclick="location='?type=3'">&nbsp; (提醒：由于本页面查询的数据量太大，严重占用服务器资源，请尽量减少刷新。)
</div>

<div class="report_tips text-info">各月同期数据对比 (每月<?php echo $day; ?>日内到院数据) (<?php echo $type_arr[intval($_GET["type"])]; ?>)</div>
