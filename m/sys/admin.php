...
if ($cur_group == 1) { //部门
	...
	foreach ($id_name as $k => $v) {
		...
		echo '<div class="rgp text-error">'.$v.'('.count($all_admin).')'.' <a href="javascript:void(0)" onclick="return sd('.$k.')">全选</a></div>';
	...
	foreach ($id_name as $k => $v) {
		...
		echo '<div id="g_'.$k.'" style="display:inline-block">';
		foreach ($all_admin as $a => $b) {
			echo '<div class="rub"><input type="checkbox" name="uid[]" value="'.$a.'" onclick="ucc(this)"><a href="/m/sys/admin.php?op=edit&id='.$b["id"].'">'.cut($b["realname"],10)." (".cut($b["realname"],10).") ".($b["isshow"]!=1 ? ' <font class="text-error">禁</font>' : '').'</a></div>';
		}
		echo '</div>';
	}
...
<b>&nbsp;&nbsp;操作：</b>
<button onclick="select_all()" type="button" class="btn">全选</button>&nbsp;
<button onclick="unselect()" type="button" class="btn">反选</button>&nbsp;

<b>&nbsp;&nbsp;所选人员：</b>
<?php if ($debug_mode || $uinfo["part_id"] == 9) { ?>
<button onclick="del()" type="button" class="btn btn-danger">删除</button>&nbsp;
<?php } ?>
<button onclick="close_account()" type="button" class="btn">关闭帐户</button>&nbsp;
<button onclick="open_account()" type="button" class="btn">开通帐户</button>&nbsp;
<button onclick="set_ch()" type="button" class="btn">批量权限</button>&nbsp;
...
?>
	...
	<button onclick="submit_ch()" class="btn">确定</button>
</span>
