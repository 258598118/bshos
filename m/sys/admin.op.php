...
if ($op == "add" || $op == "edit") {
	...
	if ($_POST) {
		...
		if ($debug_mode || $username == "admin") {
			...
			$r['purview'] = serialize(array(
					'show_tel'=>intval($_POST["show_tel"]),
					'show_patient'=>intval($_POST["show_patient"]),
					'show_patient_fee'=>intval($_POST["show_patient_fee"]),
					'show_huifang'=>intval($_POST["show_huifang"]),
					'show_chengjiao'=>intval($_POST["show_chengjiao"]),
					'show_huifang_all'=>intval($_POST["show_huifang_all"])
					));
		}
		...
		if ($db->query($sql)) {
			if ($op == "edit") {
				if ($old["part_id"] != $_POST["part_id"]) {
					//echo '<script> alert("资料提交成功");self.style.display="none";</script>';
					msg_box("资料修改成功");
				}
