...
<!-- 头部 begin -->
<div class="headers">
	<div class="headers_title"><button class="btn btn-success"><i class="icon-home icon-white"></i>操作日志列表</button></div>
	...
	<div class="headers_oprate">	    <form name="topform" method="GET">	        <?php echo $power->show_button("add"); ?>&nbsp;&nbsp;&nbsp;&nbsp;模糊搜索：<input name="searchword" value="<?php echo $_GET["searchword"]; ?>"  class="span2">&nbsp;	        <button type="submit" class="btn" value="搜索" style="font-weight:bold" title="点击搜索">搜索</button>&nbsp;	        <button type="button" value="重置" onclick="location='?'" class="btn" title="退出条件查询">重置</button>&nbsp;&nbsp;	        <button type="button" value="返回" onclick="history.back()" class="btn btn-info" title="返回上一页">返回</button>	    </form>   </div>
</div>
<!-- 头部 end -->
