...
$aTdFormat = array(
	...
	7=>array("title"=>"预诊时间", "width"=>"8%", "align"=>"center", "sort"=>"order_sort", "defaultorder"=>2),
...
<header class="jumbotron subhead"  style="margin-bottom: 20px;">
    <ul class="breadcrumb">
          ...
          <li class="active"><?=$hospital_id_name[$user_hospital_id]?> - 重复预约病人</li>
    </ul>
</header>
<div class="headers">
	<div class="header_center" style="width:65%;">
		...
		<b>起</b>: 
		<input size="16" type="text" value="" name="btime" class="start_datetime span2" id="start_datetime" value="<?php echo $_GET["btime"]; ?>"
			readonly> 
		<span class="add-on"><i class="icon-calendar"></i></span>
		<b>止</b>: 
		<input size="16" type="text" value="" name="etime" class="end_datetime span2" id="end_datetime" value="<?php echo $_GET["etime"]; ?>"
			readonly> 
		<span class="add-on"><i class="icon-calendar"></i></span>
		<b>重复项</b>: <select name="type" class="controls span2">
			<?php echo list_option(array("tel"=>"电话", "name"=>"姓名"), "_key_", "_value_", $_GET["type"]); ?>
		</select>
		<button type="submit" class="btn btn-mini" value="确定">确定</button>
		</form>
	</div>
</div>
<!-- 头部 end -->
...
?>
		<th class="head" align="<?php echo $tdalign; ?>" width="<?php echo $tdwidth; ?>"><?php echo $tdtitle; ?></th>
<? } ?>
	</thead>
	<!-- 表头定义 end -->
...
<?php
	foreach ($data2[$li[$stype]] as $line) {
		...
		$op = array();
		$op[] = "<a href='patient.php?op=view&id=$id' class='op'><img src='/static/img/b_detail.gif' align='absmiddle' title='查看' alt=''></a>";
...
?>
	<tr<?php echo $hide_line ? " class='hide'" : ""; ?> id="list_line_<?php echo $id; ?>" style="color:<?php echo $line_color[$line["status"]]; ?>">
		...
		<td align="center" class="item"><?php echo $line["author"]. ($line["edit_log"] ? ("<a href='javascript:;' title='".str_replace("<br>", "&#13", strim($line["edit_log"], '<br>'))."' style='color:#8080C0'>☆</a>") : ''); ?></td>
...
<div class="space"></div>
<div class="footer_op">
	<div class="footer_op_left"><button onclick="select_all()" class="btn">全选</button>&nbsp;<button onclick="unselect()" class="btn">反选</button>&nbsp;<?php echo $power->show_button("hide,delete"); ?></div>
