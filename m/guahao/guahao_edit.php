...
// - 创建时间 : 2013-01-29 15:57
*/
require "../../core/core.php";
$mode = 'edit';
$table = 'guahao';
$table_insert = "patient_" . $user_hospital_id;
if ($_POST)
{
    $mode = 'insert';
    ...
    if ($mode == "insert")
    {
        $oldline = $db->query("select * from $table_insert where id=$id limit 1", 1);
    } else
    {
        // 检查一个月内的病人中有无重复的:
        $name = trim($po["name"]);
        ...
        if (strlen($tel) >= 7)
        {
            ...
            $list = $db->query("select * from $table_insert where tel='$tel' and addtime>$thetime limit 1", 1);
    ...
    if (isset($po["is_local"]))
        $r["is_local"] = '2';                    // 2013-01-29  默认显示为等待
    if (isset($po["author"]))
    $r["author"] = $po["author"];
    ...
    // 导医添加直接设置为已到:
    if ($mode == "insert" && $uinfo["part_id"] == 4)
    {
        $r["status"] = 1; //已到
        $r["jiedai"] = $realname;
    }
    ...
    if ($mode == "insert")
    {
        $sql = "insert into $table_insert set $sqldata;";
        $sql2 = "update $table set `status`= 1 where id='$id' limit 1";      
    } else
    {
...
$title = $mode == "edit" ? "网站预约来源审核" : "添加新的病人资料";
...
function check_data() {
	...
	if (oForm.author.value =="") {
		oForm.author.value = <?php echo $realname;?>;
	}
	return true;
}
...
<body>
	...
	<div class="headers">
		<div class="headers_title">
			<span class="tips"><?php echo $title; ?></span>
		</div>
		<div class="header_center">
			<!-- <button onclick="if (check_data()) document.forms['mainform'].submit();" class="buttonb">提交数据</button> -->
		</div>
		<div class="headers_oprate">
			<button onClick="history.back()" class="button">返回</button>
		</div>
	</div>
	<!-- 头部 end -->
	...
	<div class="description">
		...
		<div class="d_item">1.红色部分必须填写； 2.电话号码如果填写，则必须是数字，不少于7位； 3.未尽资料填写于备注中。4.此信息来源于网站预约表单</div>
	</div>
	...
	<form name="mainform" method="POST" onSubmit="return check_data()">
		<table width="100%" class="edit">
			...
			<tr>
				<td class="left">姓名：</td>
				<td class="right"><input name="name" id="name"
					value="<?php echo $line["name"]; ?>" class="input"
					style="width: 200px" <?php echo $ce["name"]; ?>
					onchange="check_repeat('name', this)"> <span class="notice">名称必须填写</span></td>
			</tr>
			<tr>
				<td class="left">性别：</td>
				<td class="right"><input name="sex" id="sex"
					value="<?php echo $line["sex"]; ?>" class="input"
					style="width: 80px" <?php echo $ce["sex"]; ?>> <a
					href="javascript:input('sex', '男')">[男]</a> <a
					href="javascript:input('sex', '女')">[女]</a> <span class="notice">填写病人性别</span></td>
			</tr>
			<tr>
				<td class="left">年龄：</td>
				<td class="right"><input name="age" id="age"
					value="<?php echo $line["age"]; ?>" class="input"
					style="width: 80px" <?php echo $ce["age"]; ?>> <span class="notice">填写年龄</span></td>
			</tr>
<?php if ($op == "add" || ($op == "edit" && $line["author"] == $realname)) { ?>
	<tr>
				<td class="left">电话：</td>
				<td class="right"><input name="tel" id="tel"
					value="<?php echo $line["tel"]; ?>" class="input"
					style="width: 200px" <?php echo $ce["tel"]; ?>
					onchange="check_repeat('tel', this)"> <span class="intro">电话号码或手机(可不填)</span></td>
			</tr>
<?php } ?>
	<tr>
				<td class="left">QQ：</td>
				<td class="right"><input name="qq"
					value="<?php echo $line["qq"]; ?>" class="input"
					style="width: 140px" <?php echo $ce["qq"]; ?>> <span class="intro">病人QQ号码</span></td>
			</tr>
			<tr>
				<td class="left" valign="top">咨询内容：</td>
				<td class="right"><textarea name="content"
						style="width: 60%; height: 72px; vertical-align: middle;"
						<?php echo $ce["content"]; ?> class="input"><?php echo $line["content"]; ?></textarea>
					<span class="intro">咨询内容总结</span></td>
			</tr>

			<tr>
				<td class="left" valign="top">病患类型：</td>
				<td class="right"><select name="disease_id" class="combo"
			...
			</select>&nbsp;&nbsp;<span class="notice">病患类型</span></td>
			</tr>

<?php if (count($depart_list) > 0) { ?>
	<tr>
				<td class="left">所属科室：</td>
				<td class="right"><select name="depart" class="combo"
					<?php echo $ce["depart"]; ?>>
						<option value="0" style="color: gray">--请选择--</option>
				<?php echo list_option($depart_list, 'id', 'name', $line["depart"]); ?>
			</select> <span class="intro">请选择医院科室</span></td>
			</tr>
<?php } ?>

	<tr>
				<td class="left">媒体来源：</td>
				<td class="right"><select name="media_from" class="combo" value="网络挂号"
				...
				<?php echo list_option($media_from_array, '_value_', '_value_', $line["media_from"]); ?>
					...
					<?php echo list_option($engine_list, '_value_', '_value_', $line["engine"]); ?>
						...
						value="<?php echo $line["engine_key"]; ?>" class="input" size="15"
						<?php echo $ce["media_from"]; ?>> <select name="from_site"
						class="combo" <?php echo $ce["media_from"]; ?>>
			...
			<tr>
				...
				<td class="right"><select name="is_local" class="combo"
				...
				<?php echo list_option($is_local_array, '_key_', '_value_', ($op == "add" ? 1 : $line["is_local"])); ?>
						...
						value="<?php echo $line["area"]; ?>" class="input" size="14"
						<?php echo $ce["is_local"]; ?>>&nbsp; &larr;速填常用地区：<select
						id="quick_area" class="combo" <?php echo $ce["is_local"]; ?>
						onchange="byid('area').value=this.value;">
							<option value="" style="color: gray">-地区-</option>
					<?php echo list_option($area_list, "_value_", "_value_"); ?>
				</select>
				</span> <span class="notice">ip地址:<?php echo $line["city"];?></span></td>
			</tr>
		...
		<td class="right">
			...
			<tr>
				...
				<td class="right"><input name="zhuanjia_num"
					value="<?php echo $line["zhuanjia_num"]; ?>" class="input"
					size="30" style="width: 200px" <?php echo $ce["zhuanjia_num"]; ?>>
					<span class="intro"><?php echo $uinfo["part_id"] == 4 ? "就诊号" : "预约专家号"; ?></span></td>
			</tr>
			<tr>
				<td class="left" valign="top">预约时间：</td>
				<td class="right"><input name="order_date"
					value="<?php echo $line["order_date"] ? @date('Y-m-d H:i:s', $line["order_date"]) : ''; ?>"
					class="input" style="width: 150px" id="order_date"
					<?php echo $ce["order_date"]; ?>> <img src="/static/img/calendar.gif"
					id="order_date"
					onClick="picker({el:'order_date',dateFmt:'yyyy-MM-dd HH:mm:ss'})"
					align="absmiddle" style="cursor: pointer" title="选择时间"> <span
					class="intro">已修改<?php echo intval($line["order_date_changes"]); ?>次</span>
					<span class="intro">请注意，此处已调整，预约时间不能早于上个月<?php echo date("j"); ?>号，否则资料无法提交。</span><?php if ($line["order_date_log"]) { ?><a
					href="javascript:void(0)"
					onclick="byid('order_date_log').style.display = (byid('order_date_log').style.display == 'none' ? 'block' : 'none'); ">查看修改记录</a><?php } ?>
		<?php
$show_days = array (
...
?>
		...
		<div id="order_date_log" style="display: none; padding-top: 6px;">
						<b>预约时间修改记录:</b> <br><?php echo strim($line["order_date_log"], '<br>'); ?></div>
		<?php } ?>
		</td>
			</tr>
			...
			<tr>
				...
				<td class="right"><textarea name="memo"
						style="width: 60%; height: 48px; vertical-align: middle;"
						class="input" <?php echo $ce["memo"]; ?>><?php echo $line["memo"]; ?></textarea>
					<span class="intro">其他备注信息</span></td>
			</tr>
...
if (in_array($uinfo["part_id"], array (
    4, 9, 12 
)) && $line["status"] == 1)
{
    ?>
...
<?php
    ...
    ?>
<?php if (!$ce["xiangmu"]) { ?>
		<input type="hidden" name="update_xiangmu" value="1"> <span
					id="xiangmu_user"></span> <span id="xiangmu_add"><b>增加：</b><input
						id="miangmu_my_add" class="input" size="10">&nbsp;
						<button onClick="xiangmu_user_add()" class="button">确定</button></span>
					<script language="JavaScript">
...
<?php } ?>
			...
			<tr>
				...
				<td class="right"><input name="fee" id="fee"
					value="<?php echo $line["fee"] > 0 ? $line["fee"] : ''; ?>"
					class="input" <?php echo $ce["fee"]; ?> size="20"> <span
					class="intro">治疗费用</span></td>
			</tr>
<?php } ?>
...
)) && $line["status"] == 1)
{
	...
	<tr>
				...
				<td class="right"><input name="rechecktime" id="rechecktime"
					value="<?php if ($line["rechecktime"]>0) echo date("Y-m-d", $line["rechecktime"]); ?>"
					class="input" <?php echo $ce["rechecktime"]; ?> size="20">
			<?php if ($line["rechecktime"]) echo intval(($line["rechecktime"] - $line["order_date"]) / 24/3600)."天 "; ?>
			 <img src="/static/img/calendar.gif" id="order_date"
					onClick="picker({el:'rechecktime',dateFmt:'yyyy-MM-dd'})"
					align="absmiddle" style="cursor: pointer" title="选择时间"> <span
					class="intro">可填写天数(如 10 相对于预约时间推算)或具体时间(如 2009-10-1)</span></td>
			</tr>
<?php } ?>
...
<div class="space"></div>
		<table width="100%" class="edit">
			<tr>
				<td colspan="2" class="head">病人归类</td>
			</tr>
			</tr>
			<tr id="yisheng">
				<!--  style="display:<?php echo ($line["status"] == 1 ? "inline" : "none"); ?>;" -->
				<td class="left">跟踪客服：</td>
				<td class="right"><input name="author" id="author" value="<?php echo $realname;?>" class="input" style="width: 80px">&nbsp;<span class="notice">确认预约后对该病人进行跟踪,默认显示为当前用户</span></td>
			</tr>
		</table>
<?php } ?>
...
<!-- 接待记录 -->
		...
		<table width="100%" class="edit">
			...
			<tr>
				...
				<td class="right"><textarea name="jiedai_content"
						style="width: 60%; height: 48px; vertical-align: middle;"
						class="input"><?php echo $line["jiedai_content"]; ?></textarea> <span
					class="intro">请填写接待内容</span></td>
			</tr>
		</table>
<?php } ?>
...
<input type="hidden" name="id" id="id" value="<?php echo $id; ?>"> <input
			type="hidden" name="op" value="<?php echo $mode; ?>"> <input
			type="hidden" name="go" value="<?php echo $_GET["go"]; ?>">

		<div class="button_line">
			<input type="submit" class="submit" value="提交资料">&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="button" class="submit" onclick="javascript:history.back();" value="返回">
		</div>
	</form>
</body>
</html>