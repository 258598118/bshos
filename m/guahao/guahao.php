...
if ($op = $_GET["op"])
{
    ...
    {
        case "view" :
            check_power("view", $pinfo, $pagepower) or msg_box("没有新增权限...", "back", 1);
        ...
        case "delete" :
            check_power("delete",$pinfo, $pagepower)or msg_box("没有删除权限...", "back", 1);
...
// 查询条件:
$where = array ();
$where[] = "hospital_id=$user_hospital_id  and status <>1 and status <>2";
if ($searchword)
{
...
if ($sortid == 8 || ($sortid == 0 && $defaultsort == 8))
{
    ...
    {
        $data[] = array (
            "id" => 0, "name" => "今天 [" . count($data_part[1]) . "]" 
        );
    ...
    {
        $data[] = array (
            "id" => 0, "name" => "昨天 [" . count($data_part[2]) . "]" 
        );
    ...
    {
        $data[] = array (
            "id" => 0, "name" => "前天或更早 [" . count($data_part[3]) . "]" 
        );
...
<body>
	...
	<header class="jumbotron subhead"  style="margin-bottom: 20px;">
		<ul class="breadcrumb">
             ...
             <li class="active"><?=$hospital_id_name[$user_hospital_id]?> - 挂号列表</li>
        </ul>
	</header>
	<div class="headers">
		...
		<div class="headers_oprate">
			<form name="topform" method="GET">
				模糊搜索：<input name="searchword" value="<?php echo $_GET["searchword"]; ?>"  size="8">&nbsp;
				<button	type="submit" class="btn btn-small" value="搜索" style="font-weight: bold" title="点击搜索">搜索</button>&nbsp;
				<button onClick="location='?'" class="btn btn-small" title="退出条件查询">退出</button>
				&nbsp;
				<button onClick="history.back()" class="btn btn-small" title="返回上一页">返回</button>
			</form>
		</div>
	</div>
	<!-- 头部 end -->
...
foreach ( $aTdFormat as $tdid => $tdinfo )
{
    ...
    ?>
		<th class="head" align="<?php echo $tdalign; ?>"
					width="<?php echo $tdwidth; ?>"><?php echo $tdtitle; ?></th>
<? } ?>
	</tr></thread>
			<!-- 表头定义 end -->
...
if (count($data) > 0)
{
    foreach ( $data as $line )
    {
        ...
        if ($id == 0)
        {
            ?>
	<tr>
				<td colspan="<?php echo count($aTdFormat); ?>" align="left"
					class="group"><?php echo $line["name"]; ?></td>
			</tr>
<?php
        } else
        {
            ...
            if (check_power("view"))
            {
               // $op[] = "<a href='" . $pinfo["viewpage"] . "?id=$id' class='op'><img src='/static/img/b_detail.gif' align='absmiddle' title='查看' alt=''></a>";
                $op [] = "<a style='color:#111' href='guahao_view.php?op=view&id=$id' class='op'><i class='icon-eye-open'></i></a>";
                $op [] = "<a style='color:#111' href='guahao_edit.php?op=edit&id=$id&go=back'' class='op'><i class='icon-pencil'></i>";
				$op [] = "<a style='color:#111' href='?op=delete&id=$id' onclick='return isdel()' class='op'><i class='icon-trash'></i>";
            }
            if (check_power("edit"))
            {
                $op[] = "<a href='" . $pinfo["editpage"] . "?id=$id&go=back' class='op'><img src='/static/img/b_edit.gif' width='16' height='16' align='absmiddle' alt='修改'></a>";
            }
	...
	<tr <?php echo $hide_line ? " class='hide'" : ""; ?>>
				<td align="center" class="item"><input name="delcheck"
					type="checkbox" value="<?php echo $id; ?>"
					onpropertychange="set_item_color(this)"></td>
...
} else
{
    ?>
	<tr>
				<td colspan="<?php echo count($aTdFormat); ?>" align="center"
					class="nodata">(没有数据...)</td>
			</tr>
<?php } ?>
	<!-- 主要列表数据 end -->
	...
	<div class="footer_op">
		<div class="footer_op_left">
			...
			<?php echo $power->show_button("hdie,delete"); ?></div>
