...
// 更新用户在线状态:
$db->query("update sys_admin set online=if($timestamp-lastactiontime>60, '0', '1') where online=1");//用户登录时间  $addtime = $db->query_first("select addtime from sys_session where uid =$uid",'count');//1个小时超时$isexit = 0;if((intval(time())-intval($addtime))>=3600){	//$_SESSION['history'][]	$isexit = 1;}
// 读取在线用户:
$aUser = array();
$aUser[intval($uid)] = array("name"=>$username, "realname"=>$realname, "isowner"=>1,"isexit"=>$isexit);
$sqlwhere = '';
