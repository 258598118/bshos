...
;(function($, undefined) {
	...
	$.fn[pluginName] = function(options) {
		return this.each(function() {
			var obj;
			if (1+1==2) {   //这里有问题，暂时解决  fangyang 2013-5-20
				var  $this = $(this)
					,data = $this.data()
	...
	$.fn[pluginName].defaults = {
		title: '基本资料修改'		// modal title
		,target: '#modal'	// the modal id. MUST be an id for now.
		,content: ''		// the static modal content (in case it's not loaded via ajax)
		,appendTo: 'body'//parent.document.getElementsByTagName('body')	// where should the modal be appended to (default to document.body). Added for unit tests, not really needed in real life.
