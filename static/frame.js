...
// - 创建时间 : 2008-05-14 20:40 => 2009-03-30 10:38
// --------------------------------------------------------
var s_split = "";
...
function preload(image_list) {
	...
	for (var ni=0; ni<im_count; ni++) {
		im[ni] = new Image();
		im[ni].src = "/static/img/" + image_list[ni];
	}
}
...
function init_top_menu() {
	...
	for (var i in menu_mids) {
		...
		if (menu_data[mid]) {
			...
			if (has_sub_menu) {
				a_menu[ni] = "<li id='a"+mid+"'><a id='mt"+mid+"' href='"+click_link+"' onclick='load("+menu_stru[mid][0]+");return false'"; //加载第一个子菜单的链接
			} else {
				a_menu[ni] = "<li id='a"+mid+"'><a id='mt"+mid+"'  href='"+click_link+"' onclick='load("+mid+");return false'";
			}
			if (show_dyn_menu && has_sub_menu) {
				a_menu[ni] += " onmouseover='dropdownmenu(this, event, menu"+mid+", \"150px\", "+mid+")' onmouseout='delayhidemenu()'";
			}
			a_menu[ni] += " onfocus='this.blur();'>"+menu_data[mid][0]+"</a></li>";
			ni++;
...
function load(mid) {
	...
	// 顶部当前菜单加红显示:
	var e = byid("sys_top_menu").getElementsByTagName("li");
	for (var i in e) {
		e[i].className = e[i].id == ("a"+top_level_mid) ? "active" : "";
		if (e[i].id == ("a"+top_level_mid)) {
			high_light_obj = e[i];
		}
	}
	...
	if (has_sub_menu && menu_data[top_level_mid]) {
		var left_menu = "<ul class='nav nav-list'><li class='nav-header'>"+menu_data[top_level_mid][0]+"</li>";
		for (var nm in menu_stru[top_level_mid]) {
			left_menu += "<li onmouseover='mi(this)' onmouseout='mo(this)' id='mi"+menu_stru[top_level_mid][nm]+"'><a id='ml"+menu_stru[top_level_mid][nm]+"' href='"+click_link+"' onclick='load("+menu_stru[top_level_mid][nm]+");return false' class=''>"+menu_data[menu_stru[top_level_mid][nm]][0]+"</a></li>";
		}
		left_menu += "</ul>";
		byid("sys_left_menu").innerHTML = left_menu;
		byid("sys_left_menu").style.display = "block";
		if (!mid_is_top) {
			byid("mi"+mid).className = "active";
		}
	} else {
...
function show_status(string) {
	...
	if (string != '') {
		...
		//byid("sys_loading").style.left = "50%";
		//byid("sys_loading").style.marginLeft = byid("sys_loading").offsetWidth/2;
		//byid("sys_loading").style.top = "50%";
		//byid("sys_loading").style.marginTop = byid("sys_loading").offsetHeight/2;
	} else {
...
function get_online() {
	...
	online_error_timer = setTimeout("get_online_error()", 300000);
}
...
function get_online_error() {
	document.title = "请求5分钟超时,5分钟后重新连接";
	setTimeout("get_online()",300000)
}
...
function show_online_list(aOnline) {
	if (typeof(aOnline) == typeof(window)) {
		var string = "<ul width='100%' class='nav nav-list'>";
		string += "<li class='nav-header'><div style='float:left;'>在线用户</div><div style='float:right;'><a href='javascript:void(0);' onclick=\"load_url('m/sys/online_all.php')\" title='显示所有在线用户'><i class='icon-th'></i></a></div><div class='clear'></div></li>";
		for (var n in aOnline) {
			string += "<td class='item' onmouseover='mi(this)' onmouseout='mo(this)'>";
			if (n > 0) {
				string += "<li style='height:25px'><a href='#"+n+":"+aOnline[n]["name"]+"'   onclick=\"load_box(1, 'src', 'm/sys/admin.php?op=viewweb&name="+aOnline[n]["name"]+"'); return false;\" title='"+(aOnline[n]["isowner"] != 1 ? "查看用户资料" : "查看我的资料")+"'>";
			}
			...
			if (aOnline[n]["isowner"] != 1) {
				string += "&nbsp;<a href='javascript:void(0)' onclick=\"load_box(1, 'src', 'm/sys/talk.php?to="+n+"');\"  style='float:right;margin-top:-24px' class='talk' title='点击发送消息'><i class='icon-comment'></i></a>";
			}
			string + "</li>";
		}
		string += "</ul>";
...
function show_online_notice(arr) {
	if (typeof(arr) == typeof(window)) {
		var s = '';
		if (arr) {
			s += '<ul width="100%" class="nav nav-list">';
			s += '<li class="nav-header" style="color:#b94a48">通知和消息</li>';
			for (var k in arr) {
				var li = arr[k];
				s += '<li class="item" onmouseover="mi(this)" onmouseout="mo(this)">';
				if (li["type"] == "notice") {
...
function msg_box(string, showtime) {
	...
	easyDialog.open({
		  container : {
		    content : "<div style='padding:15px'>"+string+"</div>"
		  },
		  autoClose : 2000
	});
}
...
function set_body_height() {
	var all = get_size()[3];
	var main_bar_height = all  - byid("menu_bar").offsetHeight; // 6 是上下padding值
...
function load_box(isshow, type, src_or_str, params_or_title) {
	if (isshow) {
		...
		if (type == "src") {
			...
			timer_box = setInterval("reset_iframe_size()", 1000);
		...
		//document.body.removeChild(byid("dl_set_iframe"));  //删除节点
		//byid("dl_set_iframe").style.height = "auto";
...
function load_src(isshow, src) {
	if (isshow) {
		...
		//byid("dl_layer_div").style.display = "none";
		byid("dl_box_div").style = "none";
		//byid("dl_set_iframe").style.height="";
		byid("dl_set_iframe").src = "about:blank";
		try {
			clearInterval(timer_box);
		} catch (e) {
