...
/*
	...
	CREATE TABLE IF NOT EXISTS `{$ptable}` (
		...
		`order_date_changes` int(4) NOT NULL DEFAULT '0' COMMENT '预诊时间修改次数',
...
// 按钮和选择框方式的页链接
// 需要在css中定义样式 (2013-03-05 fangyang)
function pagelinkc($page, $pagecount, $reccount='-1', $linkbase='', $class='pagelink_button', $selectclass='pagelink') {
	...
	$pagelink = '<div class="pagination pagination-small">';

	// 分页数据摘要显示:
	$pagelink .= '<ul style="color:#111"><li><a>第<span class="pagelink_cur_page">'.$page.'</span>';
	...
	$pagelink .= '<ul>';
	$pagelink .= '<li><a href='."'".$base.'page='.($page-1)."'".'"'.$useful.'>&laquo;</a></li>';
	...
	if ($page <= 5)
    {
        for ($i = 1; $i <= 7; $i ++)
        {
            ...
            if ($i == 1)
            {
                if ($page != 1)
                {
                    $pagelink .= '<li><a href=' . $base . 'page=1>1</a></li>';
                } else
                {
                    $pagelink .= "<li class='active'><a href='#'>1</a></li>";
                }
            } else
            {
                $n = $i;
                if ($page != $i)
                {
                    $pagelink .= '<li><a href=' . "'" . $base . 'page=' . $n .
                             "'" . '"' . $useful . ' title="第' . $n . '页">' . $n .
                             '</a></li>';
                } else
                {
                    $pagelink .= "<li class='active'><a href='#'>{$n}</a></li>";
                }
            }
        }
    } else
    ...
    */
	...
	$pagelink .= '<li><a href='."'".$base.'page='.($page+1)."'".'"'.$useful.'>&raquo;</a></li>';
	$pagelink .= '<li><a href='."'".$base.'page='.$pagesize."'".'"'.$useful.'>末页</a></li>';
	$pagelink .='</ul>';

	// 分页下拉选择:
	
	$pagelink .= '<select name="plcombo" class="span2" onchange="location='."'".$base.'page='."'".'+this.value;" class="controls" style="margin-bottom: 22px !important;">';
...
// 加红:
function red($str) {
	return '<font class="text-error">'.$str.'</font>';
}
...
function patient_type($id)
{
	...
	return $re_arrive_array[$id]['name'];
}

//隐藏电话号码  //导医隐藏 //现场只能查看自己的    //这里有问题  过于复杂 需要重新修改
function hide_tel($telphone,$name='')
{
	global $uinfo;
	$p = substr($telphone,0,3)."****".substr($telphone,7,4);
	if(!empty($telphone))
	{
		if($purview['show_tel'] == 1) //日后去除，直接采用$show_tel
		{
			if($uinfo["part_id"]== 4)
			{
				if(preg_match("/".$uinfo['realname']."/",$name))
				{
					return  '<font class="text-error">'.$telphone.'</font>';
				}else
				{
					return $p;
				}
...
//隐藏交易金额  //除了主管、管理员 所有人只能看自己的
function hide_money($money,$name='')
{
	global $uinfo;
	if(!empty($money))
	{
		if(in_array($uinfo["part_id"],array(3,4))&&($realname!=$name))
		{
			return "****"; 
...
function format_money( $money )
{
	...
	{
		$money = number_format(intval($money),2,".",",");
	}    
	return $money;
}
