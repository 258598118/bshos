...
class table {
	...
	// 排序标记:
	var $icons = array('asc'=>'icon-arrow-down', 'desc'=>'icon-arrow-up');
	...
	function show_head() {
		...
		$s = '';
		$s .= $this->base_indent.$this->one_indent.'<thead><tr class="heddad">'."\r\n";
		foreach ($this->heads as $k => $v) {
			$ind = $this->base_indent.$this->one_indent.$this->one_indent;
			$s .= $ind.'<th'.($v["align"] ? (' align="'.$v["align"].'"') : '').
		...
		$s .= $this->base_indent.$this->one_indent.'</tr></thead>'."\r\n";
