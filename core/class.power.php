	...
	function show_button($op, $add_param='') {
		...
		foreach ($ops as $s) {
			if (@in_array($s, $c_power[$pinfo["id"]])) {
				if ($s == "add") {
					$o[] = '<button onclick="location=\'?op=add'.$add_param.'\'" class="btn" title="新增"/>新增</button>';
				}
				if ($s == "close") {
					$o[] = '<button onclick="set_show(1);return false" class="btn" title="显示">显示</button>';
					$o[] = '<button onclick="set_show(0);return false" class="btn" title="关闭">关闭</button>';
				}
				if ($s == "delete") {
					$o[] = '<button onclick="del();return false" class="btn" title="删除">删除所选</button>';
				}
			}
		}
